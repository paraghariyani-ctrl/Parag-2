      ...
      setState(() {
        events.add(
          QuotationEvent(
            ...
            event: ev,
            side: side,
            requirements: Map<String, int>.from(req),
          ),
        );
      });
    }
              ...
              children: [
                ...events.asMap().entries.map(
                  (entry) {
                    ...
                    final requirements = event.requirements.entries
                        .where((e) => e.value > 0)
                        .map((e) => '${e.key} × ${e.value}')
                        .join(' + ');
                    return Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${event.side} • ${requirements.isEmpty ? 'No requirements' : requirements}',
                        ),
                        trailing: IconButton(
                          onPressed: () => setState(() => events.removeAt(index)),
              ...
              children: [
                ...dels.asMap().entries.map(
                  (entry) {
                    ...
                    return Row(
                      children: [
                        Expanded(child: Text(deliverable.name)),
                        IconButton(
                          onPressed: () => setState(() {
                            deliverable.quantity =
                                (deliverable.quantity - 1).clamp(0, 999);
                          }),
                          icon: const Icon(Icons.remove_circle_outline),
                        ),
                        Text(
                        ...
                        IconButton(
                          onPressed: () => setState(() => deliverable.quantity++),
                          icon: const Icon(Icons.add_circle_outline),
                        ),
                        IconButton(
                          onPressed: () => setState(() => dels.removeAt(index)),
              ...
              children: [
                ...
                if (albums.isNotEmpty)
                  Align(
                    alignment: Alignment.centerRight,
                    child: Text(
                      'Album Total: ₹${albums.fold<int>(0, (sum, a) => sum + a.total)}',
...
class QuotationPdfPage extends StatelessWidget {
  ...
  pw.TextStyle get _body => const pw.TextStyle(fontSize: 8.5);
  pw.TextStyle get _small => const pw.TextStyle(fontSize: 7.5);
  pw.TextStyle get _bold => pw.TextStyle(fontSize: 8.5, fontWeight: pw.FontWeight.bold);

  Future<Uint8List> buildPdf(PdfPageFormat format) async {
    final doc = pw.Document();
    const black = PdfColor.fromInt(0xFF111111);
    const gold = PdfColor.fromInt(0xFFC49A3A);
    const lightGold = PdfColor.fromInt(0xFFF5E9CC);
    const line = PdfColor.fromInt(0xFFD5D0C7);
    const soft = PdfColor.fromInt(0xFFFAF8F4);
    ...
    final rows = quotation.events.map((e) {
      final req = e.requirements.entries
          .where((x) => x.value > 0)
          .map((x) => '${x.key} x ${x.value}')
          .join('  +  ');
      return [e.date, '${e.event} - ${e.side}', req.isEmpty ? '-' : req];
    }).toList();
    ...
    pw.Widget sectionHeader(String title) => pw.Container(
          ...
          decoration: pw.BoxDecoration(
            ...
            border: const pw.Border(
              left: pw.BorderSide(color: gold, width: 5),
            ),
          ),
          child: pw.Text(
            title.toUpperCase(),
    ...
    pw.Widget tableHeader(String text) => pw.Container(
          color: lightGold,
          padding: const pw.EdgeInsets.symmetric(horizontal: 6, vertical: 6),
          child: pw.Text(text, style: _bold, textAlign: pw.TextAlign.center),
        );
    ...
    pw.Widget infoCell(String label, String value) => pw.Padding(
          ...
          child: pw.Column(
            ...
            children: [
              pw.Text(label.toUpperCase(), style: pw.TextStyle(fontSize: 6.8, fontWeight: pw.FontWeight.bold, color: black)),
              pw.SizedBox(height: 3),
              pw.Text(value.isEmpty ? '-' : value, style: const pw.TextStyle(fontSize: 9)),
            ],
          ),
        );

    final brand = profile.name.trim().isEmpty ? 'PARAG HARIYANI FILMS' : profile.name.trim();
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        footer: (context) => pw.Container(
          ...
          decoration: const pw.BoxDecoration(border: pw.Border(top: pw.BorderSide(color: line, width: .5))),
          child: pw.Row(
            mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
            children: [
              pw.Text(profile.contact.isEmpty ? '' : profile.contact, style: _small),
              pw.Text('Page ${context.pageNumber}', style: _small),
            ],
          ),
        ),
        build: (_) => [
          ...
          pw.Container(
            ...
            decoration: const pw.BoxDecoration(border: pw.Border(bottom: pw.BorderSide(color: gold, width: 1.4))),
            child: pw.Row(
              ...
              children: [
                ...
                pw.Expanded(
                  child: pw.Column(
                    crossAxisAlignment: pw.CrossAxisAlignment.start,
                    children: [
                      pw.Text(brand.toUpperCase(), style: pw.TextStyle(fontSize: 17, fontWeight: pw.FontWeight.bold, letterSpacing: 1.1)),
                      pw.SizedBox(height: 3),
                      pw.Text('WEDDING FILMS & PHOTOGRAPHY', style: pw.TextStyle(fontSize: 7.5, letterSpacing: 2.0, color: gold, fontWeight: pw.FontWeight.bold)),
                    ],
                  ),
                ),
                pw.Column(
                  ...
                  children: [
                    pw.Text('QUOTATION', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold, letterSpacing: .8)),
                    pw.SizedBox(height: 3),
                    pw.Text('We Create Memories.', style: pw.TextStyle(fontSize: 8.5, color: gold, fontStyle: pw.FontStyle.italic)),
                  ],
                ),
              ],
            ),
          ...
          // Client information
          pw.Container(
            decoration: pw.BoxDecoration(color: soft, border: pw.Border.all(color: line, width: .6)),
            child: pw.Table(
              border: const pw.TableBorder.symmetric(inside: pw.BorderSide(color: line, width: .5)),
              columnWidths: const {0: pw.FlexColumnWidth(1), 1: pw.FlexColumnWidth(1)},
              children: [
                pw.TableRow(children: [infoCell('Client Name', quotation.client), infoCell('Quotation Date', quotation.createdDate)]),
                pw.TableRow(children: [infoCell('Contact', quotation.phone), infoCell('Email', quotation.email)]),
              ],
            ),
          ),

          sectionHeader('Event Coverage'),
          pw.Table(
            border: pw.TableBorder.all(color: line, width: .5),
            columnWidths: const {0: pw.FixedColumnWidth(58), 1: pw.FixedColumnWidth(150), 2: pw.FlexColumnWidth()},
            children: [
              pw.TableRow(children: [tableHeader('Date'), tableHeader('Event'), tableHeader('Coverage Team')]),
              ...rows.map((r) => pw.TableRow(children: r.map((v) => pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(v, style: _body))).toList())),
            ],
          ),
          ...
          pw.Table(
            ...
            children: [
              pw.TableRow(children: [tableHeader('Video Deliverables'), tableHeader('Quantity')]),
              ...quotation.deliverables.map((d) => pw.TableRow(children: [
                    pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(d.name, style: _body)),
                    pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('${d.quantity}', style: _body, textAlign: pw.TextAlign.center)),
                  ])),
            ],
          ),
          ...
          pw.Table(
            border: pw.TableBorder.all(color: line, width: .5),
            columnWidths: const {0: pw.FlexColumnWidth(3), 1: pw.FixedColumnWidth(120)},
            children: [
              pw.TableRow(children: [tableHeader('Description'), tableHeader('Amount')]),
              pw.TableRow(children: [pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('Photography & Cinematography', style: _body)), pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(money(quotation.shootCharges), style: _body, textAlign: pw.TextAlign.right))]),
              pw.TableRow(children: [pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('Video Editing Charges', style: _body)), pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(money(quotation.editingCharges), style: _body, textAlign: pw.TextAlign.right))]),
              pw.TableRow(
                decoration: const pw.BoxDecoration(color: black),
                children: [
                  pw.Padding(padding: const pw.EdgeInsets.all(7), child: pw.Text('TOTAL PACKAGE', style: pw.TextStyle(color: gold, fontSize: 9, fontWeight: pw.FontWeight.bold))),
                  pw.Padding(padding: const pw.EdgeInsets.all(7), child: pw.Text(money(quotation.total), style: pw.TextStyle(color: gold, fontSize: 9, fontWeight: pw.FontWeight.bold), textAlign: pw.TextAlign.right)),
                ],
              ),
            ],
          ),
          ...
          if (quotation.albums.isNotEmpty) ...[
            sectionHeader('Albums - Optional / Extra'),
            pw.Table(
              border: pw.TableBorder.all(color: line, width: .5),
              columnWidths: const {0: pw.FlexColumnWidth(1.7), 1: pw.FixedColumnWidth(55), 2: pw.FixedColumnWidth(70), 3: pw.FixedColumnWidth(90), 4: pw.FixedColumnWidth(90)},
              children: [
                pw.TableRow(children: [tableHeader('Album'), tableHeader('Qty'), tableHeader('Photos / Album'), tableHeader('Price / Album'), tableHeader('Total')]),
                ...quotation.albums.map((a) => pw.TableRow(children: [
                      pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(a.name, style: _small)),
                      pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('${a.quantity}', style: _small, textAlign: pw.TextAlign.center)),
                      pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text('${a.photos}', style: _small, textAlign: pw.TextAlign.center)),
                      pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(money(a.price), style: _small, textAlign: pw.TextAlign.right)),
                      pw.Padding(padding: const pw.EdgeInsets.all(6), child: pw.Text(money(a.total), style: _small, textAlign: pw.TextAlign.right)),
                    ])),
              ],
            ),
            pw.Align(
              ...
              child: pw.Container(
                ...
                child: pw.Text('Album Charges: ${money(quotation.albumTotal)} extra', style: pw.TextStyle(fontSize: 8.5, fontWeight: pw.FontWeight.bold)),
              ),
            ),
          ],

          // Always reserve a clean editable T&C area. The content comes only from the app.
          sectionHeader('Terms & Conditions'),
          pw.Container(
            width: double.infinity,
            constraints: const pw.BoxConstraints(minHeight: 72),
            padding: const pw.EdgeInsets.all(10),
            decoration: pw.BoxDecoration(border: pw.Border.all(color: line, width: .6), color: soft),
            child: quotation.tnc.trim().isEmpty
                ? pw.SizedBox(height: 52)
                : pw.Text(quotation.tnc.trim(), style: _body),
          ),

          pw.SizedBox(height: 12),
          pw.Center(child: pw.Text('Thank you for choosing ${brand.toUpperCase()}.', style: pw.TextStyle(fontSize: 9, fontWeight: pw.FontWeight.bold, letterSpacing: .5))),
          pw.SizedBox(height: 3),
          pw.Center(child: pw.Text('Wedding Films & Photography', style: pw.TextStyle(fontSize: 7.5, color: gold, letterSpacing: 1.2))),
        ],
      ),
    );
    return doc.save();
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: PdfPreview(
          ...
          allowPrinting: false,
          allowSharing: true,
          pdfFileName: '${quotation.client.isEmpty ? 'Quotation' : quotation.client.replaceAll(RegExp(r'[^A-Za-z0-9_-]+'), '_')}_Quotation.pdf',
          build: buildPdf,
        ),
      );
}

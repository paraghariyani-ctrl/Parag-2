...
class QuotationPdfPage extends StatelessWidget {
  ...
  Future<Uint8List> buildPdf(PdfPageFormat format)async{final doc=pw.Document();pw.MemoryImage? logo;if(profile.logoBase64!=null){try{logo=pw.MemoryImage(base64Decode(profile.logoBase64!));}catch(_){}}
    ...
    doc.addPage(pw.MultiPage(pageFormat:format,margin:const pw.EdgeInsets.all(30),build:(_)=>[
      ...
      pw.Column(crossAxisAlignment:pw.CrossAxisAlignment.start,children:[pw.SizedBox(height:18),pw.Text('TERMS & CONDITIONS',style:pw.TextStyle(fontSize:14,fontWeight:pw.FontWeight.bold)),pw.SizedBox(height:6),pw.Container(width:double.infinity,minHeight:88,padding:const pw.EdgeInsets.all(10),decoration:pw.BoxDecoration(border:pw.Border.all(width:.4)),child:pw.Text(quotation.tnc.trim().isEmpty?'':quotation.tnc.trim(),style:const pw.TextStyle(fontSize:8,leading:12)))]),

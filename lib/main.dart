...
class _PHFHomeState extends State<PHFHome> {
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Align(
            ...
            child: FloatingActionButton.small(heroTag: 'calendarAdd', onPressed: () => openEvent(), child: const Icon(Icons.add)),
          ),
        ),
  ...
  Widget teamTile(TeamMember m) => Card(
        child: ListTile(
          ...
          trailing: IconButton(icon: const Icon(Icons.chat_outlined), onPressed: () => chooseWhatsappEvent(m)),
        ),
      );
  ...
  Future<void> addFromContacts() async {
    ...
    final role = await chooseRole();
    if (role == null) return;
    final exists = team.any((m) => m.phone.replaceAll(RegExp(r'\D'), '') == phone.replaceAll(RegExp(r'\D'), ''));
    if (!exists) {
      team.add(TeamMember(id: DateTime.now().microsecondsSinceEpoch.toString(), name: picked.displayName, role: role, phone: phone));
      await save();
      if (mounted) setState(() {});
    }
  }

  Future<String?> chooseRole() => showDialog<String>(
        context: context,
        builder: (_) => SimpleDialog(title: const Text('Select Role'), children: roles.map((r) => SimpleDialogOption(onPressed: () => Navigator.pop(context, r), child: Text(r))).toList()),
      );

  Future<void> chooseWhatsappEvent(TeamMember member) async {
    if (events.isEmpty) {
      await whatsapp(member.phone, message: 'Hi ${member.name},\n\nPlease contact me regarding PHF event work.');
      return;
    }
    final event = await showModalBottomSheet<EventData>(
      context: context,
      builder: (_) => SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
        const Text('Send WhatsApp for event', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 10),
        ...events.map((e) => ListTile(title: Text(e.client.isEmpty ? e.name : e.client), subtitle: Text('${dateFull(e.date)} • ${e.time}'), onTap: () => Navigator.pop(context, e))),
      ])),
    );
  ...
  Future<void> openEvent({EventData? existing}) async {
    final result = await showDialog<EventData>(context: context, builder: (_) => EventDialog(existing: existing, team: team));
...
class EventDialog extends StatefulWidget {
  ...
  const EventDialog({super.key, this.existing, required this.team});
  @override
...
class _EventDialogState extends State<EventDialog> {
  ...
  void initState() {
    ...
    assignments = {for (final r in roles) r: List<String>.from(e?.assignments[r] ?? const [])};
  }
  ...
  Widget build(BuildContext context) => AlertDialog(
        title: Text(widget.existing == null ? 'Add New Event' : 'Edit Event'),
        content: SizedBox(
          width: 520,
          child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            DropdownButtonFormField<String>(value: type, decoration: const InputDecoration(labelText: 'Event Type'), items: const ['Wedding', 'Reception', 'Engagement', 'Pre Wedding', 'Other'].map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(), onChanged: (v) => setState(() => type = v ?? type)),
            TextField(controller: venue, decoration: const InputDecoration(labelText: 'Venue')),
            const SizedBox(height: 8),
            ListTile(contentPadding: EdgeInsets.zero, title: const Text('Event Time'), subtitle: Text(formatTime(time)), trailing: const Icon(Icons.access_time), onTap: () async { final t = await showTimePicker(context: context, initialTime: time); if (t != null) setState(() => time = t); }),
            ListTile(contentPadding: EdgeInsets.zero, title: const Text('Event Date'), subtitle: Text(formatDate(date)), trailing: const Icon(Icons.calendar_month), onTap: () async { final d = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2100), initialDate: date); if (d != null) setState(() => date = d); }),
            const SizedBox(height: 8),
            TextField(controller: notes, maxLines: 4, decoration: const InputDecoration(labelText: 'Notes', alignLabelWithHint: true, border: OutlineInputBorder(), hintText: 'Add event notes...')),
            ...
            const SizedBox(height: 8),
            OutlinedButton.icon(onPressed: _addRole, icon: const Icon(Icons.add), label: const Text('Add Team')),
          ])),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
          FilledButton(onPressed: _save, child: Text(widget.existing == null ? 'Save Event' : 'Save Changes')),
        ],
      );
  ...
  Widget _roleRow(String role, List<String> names) => Card(
        child: Padding(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6), child: Row(children: [
          Expanded(child: Text('$role - ${names.isEmpty ? 'Not assigned' : names.join(', ')}')),
  ...
  Future<void> _addRole() async {
    ...
    final r = await showModalBottomSheet<String>(context: context, builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: available.map((x) => ListTile(title: Text(x), onTap: () => Navigator.pop(context, x))).toList())));
    if (r == null) return;
...
class _AssignMemberDialogState extends State<AssignMemberDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final list = widget.members.where((m) => m.role.toLowerCase().contains(widget.role.toLowerCase()) || widget.role == 'Candid' || widget.role == 'Cinematographer' || q.isNotEmpty).where((m) => '${m.name} ${m.phone}'.toLowerCase().contains(q.toLowerCase())).toList();
    return AlertDialog(
      ...
      content: SizedBox(width: 420, height: 420, child: Column(children: [
        ...
        const SizedBox(height: 10),
        Expanded(child: list.isEmpty ? const Center(child: Text('No team members. Add members from Team first.')) : ListView.builder(itemCount: list.length, itemBuilder: (_, i) { final m = list[i]; return ListTile(leading: CircleAvatar(child: Text(m.name.isEmpty ? '?' : m.name[0].toUpperCase())), title: Text(m.name), subtitle: Text('${m.role} • ${m.phone}'), onTap: () => Navigator.pop(context, m)); })),
      ])),

...
class _CrewFlowHomeState extends State<CrewFlowHome> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      drawer: Drawer(child: SafeArea(child: ListView(padding: EdgeInsets.zero, children: [
        ...
        ListTile(contentPadding: const EdgeInsets.symmetric(horizontal: 20), child: Icon(Icons.work_outline, color: Colors.white)), title: const Text('CrewFlow', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)), subtitle: const Text('Organize. Assign. Capture.')),
        const Divider(),
  ...
  Widget teamTile(TeamMember m) => Card(
        child: ListTile(
          contentPadding: const EdgeInsets.symmetric(horizontal: 18, vertical: 6)), child: Text(m.name.isEmpty ? '?' : m.name[0].toUpperCase())),
      ...
      child: SizedBox(
        ...
        child: Column(children: [
          ...
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(20, 4, 20, 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 6),
            ListTile(contentPadding: EdgeInsets.zero), title: const Text('Event Time'), subtitle: Text(formatTime(time)), onTap: () async { final t = await showTimePicker(context: context, initialTime: time); if (t != null && mounted) setState(() => time = t); }),
            ListTile(contentPadding: EdgeInsets.zero), title: const Text('Event Date'), subtitle: Text(formatDate(date)), onTap: () async { final d = await showDatePicker(context: context, firstDate: DateTime(2020), lastDate: DateTime(2100), initialDate: date); if (d != null && mounted) setState(() => date = d); }),
            const SizedBox(height: 6),
...
class _QuotationEditorPageState extends State<QuotationEditorPage>{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(widget.initial == null ? 'Create Quotation' : 'Edit Quotation'), actions: [IconButton(onPressed: saveQ, icon: const Icon(Icons.check))]),
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 10, 16, 110), children: [
        _section('Client Details', Column(children: [
          TextField(controller: client, decoration: const InputDecoration(labelText: 'Client Name', prefixIcon: Icon(Icons.person_outline))),
          const SizedBox(height: 10),
        ...
        _section('Event Coverage', Column(children: [
          for (final entry in events.asMap().entries) Card(child: ListTile(
            title: Text('${entry.value.date} • ${entry.value.event}', style: const TextStyle(fontWeight: FontWeight.w700)),
            subtitle: Text('${entry.value.side} • ${entry.value.requirements.entries.where((e) => e.value > 0).map((e) => '${e.key} × ${e.value}').join(' + ')}'),
            trailing: IconButton(onPressed: () => setState(() => events.removeAt(entry.key)), icon: const Icon(Icons.delete_outline)),
          )),
          SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: addEvent, icon: const Icon(Icons.add), label: const Text('Add Event / Date'))),
        ])),
        _section('Deliverables', Column(children: [
          for (final entry in dels.asMap().entries) Row(children: [
            Expanded(child: Text(entry.value.name)),
            IconButton(onPressed: () => setState(() => entry.value.quantity = (entry.value.quantity - 1).clamp(0, 999).toInt()), icon: const Icon(Icons.remove_circle_outline)),
            Text('${entry.value.quantity}', style: const TextStyle(fontWeight: FontWeight.bold)),
            IconButton(onPressed: () => setState(() => entry.value.quantity++), icon: const Icon(Icons.add_circle_outline)),
            IconButton(onPressed: () => setState(() => dels.removeAt(entry.key)), icon: const Icon(Icons.close)),
          ]),
          DropdownButtonFormField<String>(
            decoration: const InputDecoration(labelText: 'Add Deliverable'),
            items: delOptions.where((x) => !dels.any((d) => d.name == x)).map((x) => DropdownMenuItem(value: x, child: Text(x))).toList(),
            onChanged: (x) { if (x != null) setState(() => dels.add(QuotationDeliverable(x, 1))); },
          ),
        ])),
        _section('Investment', Column(children: [
          TextField(controller: shoot, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Shoot Charges', prefixText: '₹ '), onChanged: (_) => setState(() {})),
          const SizedBox(height: 10),
          TextField(controller: editing, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Video Editing Charges', prefixText: '₹ '), onChanged: (_) => setState(() {})),
          const SizedBox(height: 12),
          Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: purple.withOpacity(.12), borderRadius: BorderRadius.circular(14)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text('Total Package', style: TextStyle(fontWeight: FontWeight.w800)), Text('₹$total', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900))])),
        ])),
        _section('Albums — Optional / Extra', Column(children: [
          for (final entry in albums.asMap().entries) Card(child: Padding(padding: const EdgeInsets.all(12), child: Column(children: [
            Row(children: [Expanded(child: TextFormField(initialValue: entry.value.name, decoration: const InputDecoration(labelText: 'Album Name'), onChanged: (v) => entry.value.name = v)), IconButton(onPressed: () => setState(() => albums.removeAt(entry.key)), icon: const Icon(Icons.delete_outline))]),
            Row(children: [
              Expanded(child: TextFormField(initialValue: '${entry.value.quantity}', keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Albums'), onChanged: (v) => setState(() => entry.value.quantity = int.tryParse(v) ?? 1))),
              const SizedBox(width: 10),
              Expanded(child: TextFormField(initialValue: '${entry.value.photos}', keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Photos / Album'), onChanged: (v) => setState(() => entry.value.photos = int.tryParse(v) ?? 0))),
              const SizedBox(width: 10),
              Expanded(child: TextFormField(initialValue: '${entry.value.price}', keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Price / Album'), onChanged: (v) => setState(() => entry.value.price = int.tryParse(v) ?? 0))),
            ]),
            Align(alignment: Alignment.centerRight, child: Text('₹${entry.value.total}', style: const TextStyle(fontWeight: FontWeight.bold))),
          ]))),
          SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: () => setState(() => albums.add(QuotationAlbum(name: 'Wedding Album', quantity: 1, price: 0, photos: 400))), icon: const Icon(Icons.add), label: const Text('Add Album'))),
          if (albums.isNotEmpty) Align(alignment: Alignment.centerRight, child: Text('Album Total: ₹${albums.fold(0, (sum, a) => sum + a.total)}', style: const TextStyle(fontWeight: FontWeight.w800))),
        ])),
        _section('Terms & Conditions', Column(children: [
          if (tcs.isNotEmpty) DropdownButtonFormField<String>(decoration: const InputDecoration(labelText: 'Select Saved T&C'), items: tcs.asMap().entries.map((entry) => DropdownMenuItem(value: entry.value, child: Text('Template ${entry.key + 1}', overflow: TextOverflow.ellipsis))).toList(), onChanged: (v) { if (v != null) setState(() => tnc.text = v); }),
          if (tcs.isNotEmpty) const SizedBox(height: 8),
          TextField(controller: tnc, maxLines: 7, decoration: const InputDecoration(hintText: 'Write T&C for this quotation...', border: OutlineInputBorder())),
          Align(alignment: Alignment.centerLeft, child: TextButton.icon(onPressed: manageTcs, icon: const Icon(Icons.save_outlined), label: const Text('Save as new T&C template'))),
        ])),
        const SizedBox(height: 16),
        FilledButton.icon(onPressed: saveQ, icon: const Icon(Icons.save), label: const Text('Save Quotation & Open PDF')),
      ]),
    );
  }

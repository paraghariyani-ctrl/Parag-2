...
class _QuotationEditorPageState extends State<QuotationEditorPage>{
  ...
  @override
  Widget build(BuildContext context) {
    final eventCards = events.asMap().entries.map<Widget>((entry) {
      final index = entry.key;
      final event = entry.value;
      final coverage = event.requirements.entries
          .where((e) => e.value > 0)
          .map((e) => '${e.key} × ${e.value}')
          .join(' + ');
      ...
      return Card(
        child: ListTile(
          ...
          subtitle: Text(
            '${event.side} • ${coverage.isEmpty ? 'No team selected' : coverage}',
          ),
          trailing: IconButton(
    ...
    final deliverableRows = dels.asMap().entries.map<Widget>((entry) {
      final index = entry.key;
      ...
      return Row(
        children: [
          ...
          IconButton(
            onPressed: () => setState(() {
              item.quantity = (item.quantity - 1).clamp(0, 999).toInt();
            }),
    ...
    final albumCards = albums.asMap().entries.map<Widget>((entry) {
      final index = entry.key;
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      onChanged: (value) => album.name = value,
                    ),
                  ),
                  IconButton(
                    onPressed: () => setState(() => albums.removeAt(index)),
                    icon: const Icon(Icons.delete_outline),
                  ),
                ],
              ),
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      onChanged: (value) {
                        album.quantity = int.tryParse(value) ?? 1;
                        setState(() {});
                      },
                    ),
                  ),
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: const InputDecoration(labelText: 'Photos / Album'),
                      onChanged: (value) {
                        album.photos = int.tryParse(value) ?? 0;
                        setState(() {});
                      },
                    ),
                  ),
                  ...
                  Expanded(
                    child: TextFormField(
                      ...
                      keyboardType: TextInputType.number,
                      decoration: const InputDecoration(labelText: 'Price / Album'),
                      onChanged: (value) {
                        album.price = int.tryParse(value) ?? 0;
                        setState(() {});
                      },
                    ),
                  ),
    ...
    final tcDropdown = tcs.isEmpty
        ? const <Widget>[]
        : <Widget>[
            DropdownButtonFormField<String>(
              decoration: const InputDecoration(labelText: 'Select Saved T&C'),
              items: tcs.asMap().entries.map<DropdownMenuItem<String>>((entry) {
            ...
            Column(
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: delOptions
                      .where((name) => !dels.any((d) => d.name == name))
                  ...
                  onChanged: (value) {
                    if (value != null) {
                      setState(() => dels.add(QuotationDeliverable(value, 1)));
                    }
                  },
                ),
              ],
              ...
              children: [
                ...
                if (albums.isNotEmpty)
                  Align(
                    alignment: Alignment.centerRight,
                    child: Text(
                      'Album Total: ₹${albums.fold<int>(0, (sum, album) => sum + album.total)}',

    ...
    body:SafeArea(child:Center(child:SingleChildScrollView(
      ...
      child:ConstrainedBox(
        ...
        child:Column(children:[
          Container(width:82,height:82,decoration:BoxDecoration(color:purple.withOpacity(.16),borderRadius:BorderRadius.circular(24)),child:const Icon(Icons.auto_awesome_rounded,color:purple,size:42)),
          const SizedBox(height:18),
          const Text('CrewFlow',style:TextStyle(fontSize:32,fontWeight:FontWeight.w900)),
          const SizedBox(height:6),
          const Text('Organize. Assign. Capture.',style:TextStyle(color:Colors.grey)),
          const SizedBox(height:42),
          Align(alignment:Alignment.centerLeft,child:Text(create?'Create your account':'Welcome back',style:const TextStyle(fontSize:24,fontWeight:FontWeight.w800))),
          const SizedBox(height:18),
          ...
          SizedBox(width:double.infinity,height:52,child:FilledButton(onPressed:_submit,child:Text(create?'Create Account':'Login'))),
          const SizedBox(height:14),
          Row(children:[const Expanded(child:Divider()),Padding(padding:const EdgeInsets.symmetric(horizontal:12),child:Text('or')),const Expanded(child:Divider())]),
          const SizedBox(height:14),
          SizedBox(width:double.infinity,height:50,child:OutlinedButton.icon(onPressed:_submit,icon:const Icon(Icons.g_mobiledata,size:30),label:const Text('Continue with Google'))),
          const SizedBox(height:16),
          TextButton(onPressed:()=>setState(()=>create=!create),child:Text(create?'Already have an account? Login':'Don’t have an account? Create one')),
          const SizedBox(height:18),
          const Text('Login keeps your CrewFlow session on this device. Cloud restore needs a connected backend.',textAlign:TextAlign.center,style:TextStyle(fontSize:11,color:Colors.grey))
        ])
      )
    )))
  );
...
class _EventDialogState extends State<EventDialog> {
  ...
  String roleLabel(String role) { final base = role.replaceFirst(RegExp(r' #\d+$'), ''); return base == 'Cinematographer' ? 'Cinematic' : base; }
  ...
  Future<void> _addRole() async {
    // Unlimited roles: every tap creates a new slot, e.g. Cinematic, Cinematic #2, Cinematic #3.
    ...
    if (assignments.containsKey(slot)) {
      var n = 2;
      while (assignments.containsKey('$r #$n')) n++;
      slot = '$r #$n';
    }
    ...
    setState(() {
      assignments[slot] = selected.map((m) => m.name).toSet().toList();
    });
  }
...
class _QuotationListPageState extends State<QuotationListPage>{
  ...
  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('Quotations'),actions:[IconButton(onPressed:()=>edit(),icon:const Icon(Icons.add))]),floatingActionButton:FloatingActionButton(onPressed:()=>edit(),child:const Icon(Icons.add)),body:qs.isEmpty?const Center(child:Column(mainAxisSize:MainAxisSize.min,children:[Icon(Icons.request_quote_outlined,size:54),SizedBox(height:12),Text('No quotations yet',style:TextStyle(fontSize:18,fontWeight:FontWeight.bold)),SizedBox(height:6),Text('Create your first professional quotation')])):ListView.builder(padding:const EdgeInsets.all(12),itemCount:qs.length,itemBuilder:(_,i){final q=qs[i];return Card(child:ListTile(contentPadding:const EdgeInsets.all(14),leading:CircleAvatar(backgroundColor:purple.withOpacity(.18),child:const Icon(Icons.request_quote_outlined,color:purple)),title:Text(q.client.isEmpty?'Untitled Quotation':q.client,style:const TextStyle(fontWeight:FontWeight.w700)),subtitle:Text('${q.events.length} event${q.events.length==1?'':'s'} • ${q.createdDate}\n₹${q.total.toString()}'),isThreeLine:true,trailing:PopupMenuButton<String>(onSelected:(v)async{if(v=='edit')await edit(q);if(v=='pdf')await Navigator.push(context,MaterialPageRoute(builder:(_)=>QuotationPdfPage(quotation:q,profile:widget.profile)));if(v=='delete'){setState(()=>qs.removeWhere((x)=>x.id==q.id));await persist();}},itemBuilder:(_)=>const[PopupMenuItem(value:'edit',child:Text('Edit')),PopupMenuItem(value:'pdf',child:Text('Open PDF')),PopupMenuItem(value:'delete',child:Text('Delete'))])));}}));
}
...
class _QuotationEditorPageState extends State<QuotationEditorPage>{
  ...
  Future<void> addEvent()async{DateTime d=DateTime.now();String ev=eventOptions.first,side='Both Side';final req=<String,int>{};final r=await showDialog<bool>(context:context,builder:(_)=>StatefulBuilder(builder:(ctx,setD)=>AlertDialog(title:const Text('Add Event'),content:SizedBox(width:480,child:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[ListTile(leading:const Icon(Icons.calendar_month),title:Text(fmt(d)),trailing:const Icon(Icons.chevron_right),onTap:()async{final x=await showDatePicker(context:ctx,firstDate:DateTime(2020),lastDate:DateTime(2100),initialDate:d);if(x!=null)setD(()=>d=x);}),DropdownButtonFormField<String>(value:ev,decoration:const InputDecoration(labelText:'Event'),items:eventOptions.map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(x)=>setD(()=>ev=x!),),DropdownButtonFormField<String>(value:side,decoration:const InputDecoration(labelText:'Side'),items:const['Bride Side','Groom Side','Both Side'].map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(x)=>setD(()=>side=x!),),const SizedBox(height:10),const Align(alignment:Alignment.centerLeft,child:Text('Requirements',style:TextStyle(fontWeight:FontWeight.bold))),...reqOptions.map((x)=>Row(children:[Expanded(child:Text(x)),IconButton(onPressed:()=>setD(()=>req[x]=((req[x]??0)-1).clamp(0,99)),icon:const Icon(Icons.remove_circle_outline)),Text('${req[x]??0}',style:const TextStyle(fontWeight:FontWeight.bold)),IconButton(onPressed:()=>setD(()=>req[x]=(req[x]??0)+1),icon:const Icon(Icons.add_circle_outline))]))])),),actions:[TextButton(onPressed:()=>Navigator.pop(ctx,false),child:const Text('Cancel')),FilledButton(onPressed:()=>Navigator.pop(ctx,true),child:const Text('Add'))]));if(r==true){setState(()=>events.add(QuotationEvent(date:fmt(d),event:ev,side:side,requirements:Map.from(req))));}}
  Future<void> manageTcs()async{final c=TextEditingController();final r=await showDialog<String>(context:context,builder:(_)=>AlertDialog(title:const Text('Add T&C Template'),content:TextField(controller:c,maxLines:7,decoration:const InputDecoration(hintText:'Write your terms & conditions here...')),actions:[TextButton(onPressed:()=>Navigator.pop(context),child:const Text('Cancel')),FilledButton(onPressed:()=>Navigator.pop(context,c.text.trim()),child:const Text('Save'))]));if(r!=null&&r.trim().isNotEmpty){setState(()=>tcs.add(r.trim()));widget.onTemplatesChanged(tcs);setState(()=>tnc.text=r.trim());}}
  Future<void> saveQ() async {if(client.text.trim().isEmpty){ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content:Text('Please enter Client Name.')));return;}final q=Quotation(id:widget.initial?.id??DateTime.now().microsecondsSinceEpoch.toString(),client:client.text.trim(),phone:phone.text.trim(),email:email.text.trim(),createdDate:widget.initial?.createdDate??fmt(DateTime.now()),events:events,deliverables:dels,shootCharges:num(shoot),editingCharges:num(editing),albums:albums,tnc:tnc.text.trim());await Navigator.push(context,MaterialPageRoute(builder:(_)=>QuotationPdfPage(quotation:q,profile:widget.profile)));if(mounted)Navigator.pop(context,q);}
  @override Widget build(BuildContext context){return Scaffold(appBar:AppBar(title:Text(widget.initial==null?'Create Quotation':'Edit Quotation'),actions:[IconButton(onPressed:saveQ,icon:const Icon(Icons.check))]),body:ListView(padding:const EdgeInsets.fromLTRB(16,10,16,110),children:[_section('Client Details',Column(children:[TextField(controller:client,decoration:const InputDecoration(labelText:'Client Name',prefixIcon:Icon(Icons.person_outline))),const SizedBox(height:10),TextField(controller:phone,keyboardType:TextInputType.phone,decoration:const InputDecoration(labelText:'Mobile Number',prefixIcon:Icon(Icons.phone_outlined))),const SizedBox(height:10),TextField(controller:email,keyboardType:TextInputType.emailAddress,decoration:const InputDecoration(labelText:'Email (optional)',prefixIcon:Icon(Icons.email_outlined)))])),_section('Event Coverage',Column(children:[...events.asMap().entries.map((x)=>Card(child:ListTile(title:Text('${x.value.date} • ${x.value.event}',style:const TextStyle(fontWeight:FontWeight.w700)),subtitle:Text('${x.value.side} • ${x.value.requirements.entries.where((e)=>e.value>0).map((e)=>'${e.key} × ${e.value}').join(' + ')}'),trailing:IconButton(onPressed:()=>setState(()=>events.removeAt(x.key)),icon:const Icon(Icons.delete_outline)))),SizedBox(width:double.infinity,child:OutlinedButton.icon(onPressed:addEvent,icon:const Icon(Icons.add),label:const Text('Add Event / Date'))])),_section('Deliverables',Column(children:[...dels.asMap().entries.map((x)=>Row(children:[Expanded(child:Text(x.value.name)),IconButton(onPressed:()=>setState(()=>x.value.quantity=(x.value.quantity-1).clamp(0,999)),icon:const Icon(Icons.remove_circle_outline)),Text('${x.value.quantity}',style:const TextStyle(fontWeight:FontWeight.bold)),IconButton(onPressed:()=>setState(()=>x.value.quantity++),icon:const Icon(Icons.add_circle_outline)),IconButton(onPressed:()=>setState(()=>dels.removeAt(x.key)),icon:const Icon(Icons.close))])),DropdownButtonFormField<String>(decoration:const InputDecoration(labelText:'Add Deliverable'),items:delOptions.where((x)=>!dels.any((d)=>d.name==x)).map((x)=>DropdownMenuItem(value:x,child:Text(x))).toList(),onChanged:(x){if(x!=null)setState(()=>dels.add(QuotationDeliverable(x,1)));})])),_section('Investment',Column(children:[TextField(controller:shoot,keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'Shoot Charges',prefixText:'₹ '),onChanged:(_)=>setState((){})),const SizedBox(height:10),TextField(controller:editing,keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'Video Editing Charges',prefixText:'₹ '),onChanged:(_)=>setState((){})),const SizedBox(height:12),Container(width:double.infinity,padding:const EdgeInsets.all(16),decoration:BoxDecoration(color:purple.withOpacity(.12),borderRadius:BorderRadius.circular(14)),child:Row(mainAxisAlignment:MainAxisAlignment.spaceBetween,children:[const Text('Total Package',style:TextStyle(fontWeight:FontWeight.w800)),Text('₹${total.toString()}',style:const TextStyle(fontSize:18,fontWeight:FontWeight.w900))]))])),_section('Albums — Optional / Extra',Column(children:[...albums.asMap().entries.map((x)=>Card(child:Padding(padding:const EdgeInsets.all(12),child:Column(children:[Row(children:[Expanded(child:TextField(controller:TextEditingController(text:x.value.name),decoration:const InputDecoration(labelText:'Album Name'),onChanged:(v)=>x.value.name=v)),IconButton(onPressed:()=>setState(()=>albums.removeAt(x.key)),icon:const Icon(Icons.delete_outline))]),Row(children:[Expanded(child:TextField(controller:TextEditingController(text:'${x.value.quantity}'),keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'Albums'),onChanged:(v)=>x.value.quantity=int.tryParse(v)??1)),const SizedBox(width:10),Expanded(child:TextField(controller:TextEditingController(text:'${x.value.photos}'),keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'Photos / Album'),onChanged:(v)=>x.value.photos=int.tryParse(v)??0)),const SizedBox(width:10),Expanded(child:TextField(controller:TextEditingController(text:'${x.value.price}'),keyboardType:TextInputType.number,decoration:const InputDecoration(labelText:'Price / Album'),onChanged:(v)=>x.value.price=int.tryParse(v)??0))]),Align(alignment:Alignment.centerRight,child:Text('₹${x.value.total}',style:const TextStyle(fontWeight:FontWeight.bold)))]))),SizedBox(width:double.infinity,child:OutlinedButton.icon(onPressed:()=>setState(()=>albums.add(QuotationAlbum(name:'Wedding Album',quantity:1,price:0,photos:400))),icon:const Icon(Icons.add),label:const Text('Add Album'))),if(albums.isNotEmpty)Align(alignment:Alignment.centerRight,child:Text('Album Total: ₹${albums.fold(0,(s,a)=>s+a.total)}',style:const TextStyle(fontWeight:FontWeight.w800)))])),_section('Terms & Conditions',Column(children:[if(tcs.isNotEmpty)DropdownButtonFormField<String>(decoration:const InputDecoration(labelText:'Select Saved T&C'),items:tcs.asMap().entries.map((x)=>DropdownMenuItem(value:x.value,child:Text('Template ${x.key+1}',overflow:TextOverflow.ellipsis))).toList(),onChanged:(v){if(v!=null)setState(()=>tnc.text=v);}),if(tcs.isNotEmpty)const SizedBox(height:8),TextField(controller:tnc,maxLines:7,decoration:const InputDecoration(hintText:'Write T&C for this quotation...',border:OutlineInputBorder())),Align(alignment:Alignment.centerLeft,child:TextButton.icon(onPressed:manageTcs,icon:const Icon(Icons.save_outlined),label:const Text('Save as new T&C template'))])),const SizedBox(height:16),FilledButton.icon(onPressed:saveQ,icon:const Icon(Icons.save),label:const Text('Save Quotation & Open PDF'))]));}
...
class QuotationPdfPage extends StatelessWidget {
  ...
  Future<Uint8List> buildPdf(PdfPageFormat format)async{final doc=pw.Document();pw.MemoryImage? logo;if(profile.logoBase64!=null){try{logo=pw.MemoryImage(base64Decode(profile.logoBase64!));}catch(_){}}
    ...
    doc.addPage(pw.MultiPage(pageFormat:format,margin:const pw.EdgeInsets.all(30),build:(_)=>[
      ...
      if(quotation.tnc.trim().isNotEmpty)pw.Column(crossAxisAlignment:pw.CrossAxisAlignment.start,children:[pw.SizedBox(height:18),pw.Text('TERMS & CONDITIONS',style:pw.TextStyle(fontSize:14,fontWeight:pw.FontWeight.bold)),pw.SizedBox(height:6),pw.Container(width:double.infinity,padding:const pw.EdgeInsets.all(10),decoration:pw.BoxDecoration(border:pw.Border.all(width:.4)),child:pw.Text(quotation.tnc,style:const pw.TextStyle(fontSize:8,leading:12)))]),

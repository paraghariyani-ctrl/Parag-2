...
class _AssignMemberDialogState extends State<AssignMemberDialog> {
  ...
  @override
  Widget build(BuildContext context) {
    final wantedRole = widget.role == 'Cinematographer' ? 'Cinematographer' : widget.role;
    final list = widget.members.where((m) {
      final roleMatches = m.role.toLowerCase().contains(wantedRole.toLowerCase()) ||
          (wantedRole == 'Cinematographer' && m.role.toLowerCase().contains('cinematic'));
      final queryMatches = '${m.name} ${m.phone}'.toLowerCase().contains(q.toLowerCase());
      return q.isNotEmpty ? queryMatches : roleMatches;
    }).toList();
    return AlertDialog(
